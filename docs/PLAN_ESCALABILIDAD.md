...
4. **Migración bucket:** ¿qué objetos exactos de `eig-chatbot-logs-prod` migramos? **BLOQUEADO:** la credencial `raw_bucket` (cuenta `092443462435`) del `.env` da AccessDenied en todo (origen y destino); se necesitan creds con `s3:ListBucket`+`s3:GetObject` en origen y `+s3:PutObject` en destino para inspeccionar y migrar.
...
- [~] **Paso 8 — Migración bucket.** `scripts/migrate_bucket.py` entregado y testeado (idempotente, copy-only, dry-run por defecto). **Ejecución BLOQUEADA:** la credencial `raw_bucket` del `.env` no tiene permisos (AccessDenied en `ListBucket`/`GetObject` sobre origen y destino). Falta: (a) credenciales con permisos, (b) confirmar qué migrar (el `default_map_key` es identidad — hoy no se conoce el layout del bucket viejo).
