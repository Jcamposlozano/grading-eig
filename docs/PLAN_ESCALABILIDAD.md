...
- [x] **Paso 11 — Deprecación v1 (marcadas, no eliminadas).** Rutas v1 (`/v1/materials*`, `/v1/evaluations`) marcadas `deprecated=True` en OpenAPI + middleware que añade headers `Deprecation: true` / `Link: </v1/calificaciones>; rel="successor-version"` en **todas** sus respuestas (incl. errores). Siguen funcionales; sucesor: `POST /v1/calificaciones`. Eliminación futura. 83 tests verdes.
